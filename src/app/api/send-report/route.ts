...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Diëtiek rapport verzonden naar ${email} voor ${name}`);
    return NextResponse.json({ success: true });

  } catch (error) {
    console.error('Error sending Diëtiek report:', error);
    return NextResponse.json(
  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background-color: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
        ...
        <p>Bedankt voor het invullen van de Diëtiek vragenlijst. Hieronder vind je een samenvatting van je antwoorden en wat dit kan betekenen voor jouw situatie.</p>
...
Bedankt voor het invullen van de Diëtiek vragenlijst. Hieronder vind je een samenvatting van je antwoorden en wat dit kan betekenen voor jouw situatie.
